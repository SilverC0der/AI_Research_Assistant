...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for model_name, config in data_driven_config.items():
            obj, created = ModelReliability.objects.update_or_create(
                model_name=model_name, defaults=config
            )
            ...
            try:
                ...
                if perf:
                    metrics[model_name] = {
                        ...
                        "format_compliance_score": getattr(
                            perf, "format_compliance_score", 0.0
                        ),
                        "format_compliance_count": getattr(
                            perf, "format_compliance_count", 0
                        ),
                        "format_compliance_passed": getattr(
                            perf, "format_compliance_passed", 0
                        ),
    ...
    def generate_data_driven_config(self, performance_data):
        ...
        sorted_models = sorted(
            ...
            key=lambda x: (
                ...
                (
                    -x[1]["avg_response_time"] if x[1]["avg_response_time"] > 0 else 0
                ),  # Tertiary: faster is better
        ...
        for i, (model_name, metrics) in enumerate(sorted_models):
            ...
            # No data yet - start strict
            if metrics["format_compliance_count"] == 0:
                custom_temperature = 0.2
            elif metrics["format_compliance_score"] >= 0.8:
                custom_temperature = 0.2
    ...
    def show_performance_insights(self, performance_data):
        ...
        if active_models:
            ...
            avg_format_compliance = (
                sum(
                    ...
                    if m["format_compliance_count"] > 0
                )
                / len([m for m in active_models if m["format_compliance_count"] > 0])
            ...
            avg_response_time = (
                ...
                / len([m for m in active_models if m["avg_response_time"] > 0])
                if any(m["avg_response_time"] > 0 for m in active_models)
                else 0
            )

            self.stdout.write(f"  Average reliability score: {avg_reliability:.3f}")
            self.stdout.write(
                f"  Average format compliance: {avg_format_compliance:.1%}"
            )
        ...
        models_with_format_data = [
            ...
            if d["format_compliance_count"] > 0
        ]
        if models_with_format_data:
            ...
            self.stdout.write("\nTop 3 Models by Format Compliance:")
            self.stdout.write(
                "  No format compliance data available yet - make some summary requests!"
            )

        # Models needing more data
        models_needing_data = [
            (m, d) for m, d in performance_data.items() if d["total_requests"] < 5
        ]
        if models_needing_data:
            ...
            for model_name, metrics in models_needing_data[:5]:  # Show first 5
                self.stdout.write(
                    f'  - {model_name}: {metrics["total_requests"]} requests'
                )
